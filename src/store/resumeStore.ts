...
const defaultData: ResumeData = {
  ...
  work: [
    {
      ...
      highlights:
        'Reduced bundle size by 45%;Improved LCP from 4s to 0.9s;Built design system used across 5 products',
    },
    {
      id: 'w2',
      company: 'Startup XYZ',
  ...
  education: [
    {
      id: 'e1',
      school: 'Tashkent University of IT',
    ...
    (set) => ({
      ...
      addWork: () =>
        set((s) => ({
          data: {
            ...
            work: [
              ...s.data.work,
              { id: uid(), company: '', role: '', period: '', description: '', highlights: '' },
            ],
          },
        })),
      removeWork: (id) =>
      ...
      updateEdu: (id, p) =>
        set((s) => ({
          data: {
            ...s.data,
            education: s.data.education.map((e) => (e.id === id ? { ...e, ...p } : e)),
          },
        })),
      addEdu: () =>
        set((s) => ({
          data: {
            ...s.data,
            education: [...s.data.education, { id: uid(), school: '', degree: '', year: '' }],
          },
        })),
      removeEdu: (id) =>
        set((s) => ({
      ...
      updateProject: (id, p) =>
        set((s) => ({
          data: {
            ...s.data,
            projects: s.data.projects.map((pr) => (pr.id === id ? { ...pr, ...p } : pr)),
          },
        })),
      addProject: () =>
        set((s) => ({
      ...
      updateSkillGroup: (id, p) =>
        set((s) => ({
          data: {
            ...s.data,
            skillGroups: s.data.skillGroups.map((sg) => (sg.id === id ? { ...sg, ...p } : sg)),
          },
        })),
      addSkillGroup: () =>
        set((s) => ({
          data: {
            ...s.data,
            skillGroups: [...s.data.skillGroups, { id: uid(), category: '', skills: '' }],
          },
        })),
      removeSkillGroup: (id) =>
        set((s) => ({
          data: { ...s.data, skillGroups: s.data.skillGroups.filter((sg) => sg.id !== id) },
        })),
