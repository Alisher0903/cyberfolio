...
export default function ResumePreview({ data, scale = 1, forExport = false }: Props) {
  ...
  const Section = ({ title, children }: { title: string; children: React.ReactNode }) => (
    <section style={{ marginBottom: '1.5rem' }}>
      {isTerm ? (
        ...
        <h2
          style={{
            fontFamily: isMin ? "'DM Sans',sans-serif" : "'Syne',sans-serif",
            fontSize: isMin ? '0.6rem' : '0.65rem',
            fontWeight: 700,
  ...
  return (
    <div id="resume-preview" style={containerStyle}>
      ...
      >
        ...
        >
          ...
          >
            {data.email && (
              <div>
                {isTerm && <span style={{ color: theme.dim }}>email: </span>}
                {data.email}
              </div>
            )}
            {data.phone && (
              <div>
                {isTerm && <span style={{ color: theme.dim }}>tel: </span>}
                {data.phone}
              </div>
            )}
            {data.location && (
              <div>
                {isTerm && <span style={{ color: theme.dim }}>loc: </span>}
                {data.location}
              </div>
            )}
      ...
      {/* ── TWO-COLUMN BODY ─────────────────── */}
      <div
        style={{ display: 'grid', gridTemplateColumns: isMin ? '1fr' : '2fr 1fr', gap: '1.5rem' }}
      >
        {/* Main column */}
        <div>
          {/* Work Experience */}
            ...
            <Section title="Projects">
              {data.projects.map((p) => (
                <div key={p.id} style={{ marginBottom: '0.8rem' }}>
                  ...
                  {p.description && (
                    <p style={{ fontSize: '0.62rem', color: isTerm ? '#7FA8C4' : '#555' }}>
                      {p.description}
                    </p>
                  )}
                </div>
                    ...
                    {sg.skills
                      ...
                      .map((sk, i) => (
                        <span key={i}>
                          {isTerm ? (
                            <span>
                              <span style={{ color: theme.primary, marginRight: '0.2rem' }}>›</span>
                              {sk.trim()}{' '}
                            </span>
                          ) : (
                            <span
